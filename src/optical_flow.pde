...
int wscreen=app_width;
int hscreen=app_height;
...
void of_setup(){
  // screen and video
  if (cap_mov) {
    video_cap = new Capture(this, wscreen, hscreen, fps);
    video_cap.start();
  } else {
    video_mov = new Movie(this, movie_file);
    //video_mov.width = wscreen;
    //video_mov.height = hscreen;
    video_mov.play();
  }
  // font
...
void pixave(int x1, int y1, int x2, int y2) {
  ...
  for(int y=y1; y<=y2; y++) {
    for(int i=wscreen*y+x1; i<=wscreen*y+x2; i++) {
      pix=(cap_mov) ? video_cap.pixels[i] : video_mov.pixels[i];
      b=pix & 0xFF; // blue
      pix = pix >> 8;
      g=pix & 0xFF; // green
      pix = pix >> 8;
...
void of_draw() {
  ...
  if((cap_mov) ? video_cap.available() : video_mov.available()){
    // video capture
    if (cap_mov) {
      video_cap.read();
    }else {
    ...
    // mirror
    if(flagmirror && cap_mov==true) {
      for(int y=0;y<hscreen;y++) {
        int ig=y*wscreen;
        for(int x=0; x<wscreen; x++) 
          vline[x] = (cap_mov) ? video_cap.pixels[ig+x] : video_mov.pixels[ig+x];
        for(int x=0; x<wscreen; x++) {
          if (cap_mov) {
            video_cap.pixels[ig+x]=vline[wscreen-1-x];
          }else {
            video_mov.pixels[ig+x]=vline[wscreen-1-x];
          }
        }
      }
    }
...
/*    
    ...
    if(flagimage) image((cap_mov) ? video_cap : video_mov, 0, 0);
    else background(0);
*/
...
//  if(flagmovie) movie.addFrame();
  if(flagmovie) saveFrame(rec_file);
